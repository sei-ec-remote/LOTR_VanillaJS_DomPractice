...
function forgeTheFellowShip() {
	...
	for (let i = 0; i < 4; i++) {
		theFellowshipDiv.appendChild(ulHobbits[0])
		alert(`${hobbits[i]} has joined The Fellowship.`)
	}	
	for (let i = 0; i < 5; i++) {
		theFellowshipDiv.appendChild(ulBuddies[0])
...
function theBalrog() {
	...
	const gandalf = fellowship.item(4)
	console.log(gandalf)
...
function thereAndBackAgain() {
	...
	const theShire = document.getElementById('The-Shire')
	const ulHobbits = document.createElement('ul')
	ulHobbits.id = 'hobbit-list'
	const waywardHobbits = document.querySelectorAll('.hobbit')
	console.log(waywardHobbits)
	waywardHobbits.forEach(hob => {
		ulHobbits.appendChild(hob)
	})
	theShire.appendChild(ulHobbits)
}
