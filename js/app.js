...
function makeMiddleEarth() {
	...
	//   2a. creates an article tag (there should be one for each land when the loop is done)
		const newLandArticle = document.createElement("article");

	//   2b. gives each land article an `id` tag of the corresponding land name
		newLandArticle.setAttribute("id", lands[i]);

	//   2c. includes an h1 with the name of the land inside each land article
		newLandArticle.innerHTML = `<h1>${lands[i]}</h1>`;

	//   2d. appends each land to the middle-earth section
		middleEarth.appendChild(newLandArticle);
	}
...
function makeBaddies() {
	...
	document.querySelector("#Mordor").appendChild(baddieList);
}
...
function theBalrog() {
	...
	const gandalf = document.querySelector(`[id='Gandalf the Grey']`);
...
function weWantsIt() {
	...
	gollum.appendChild(document.querySelector(".magic-imbued-jewelry"));
...
function thereAndBackAgain() {
	...
	document.querySelector("#gollum").removeChild(document.querySelector(".magic-imbued-jewelry"));
	document.querySelector("#mount-doom").removeChild(document.querySelector("#gollum"));
	document.querySelectorAll(".hobbit").forEach(
		function (hobbit) { document.querySelector("#The-Shire").appendChild(hobbit); }
	);
}
